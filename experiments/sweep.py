...
def get_experiment_configs():
    ...
    for model, dataset, seed, bit_version in itertools.product(MODELS, DATASETS, SEEDS, VERSIONS):
        yield {
            "model": model,
            "dataset": dataset,
            "seed": seed,
...
def run_experiment(config: dict, output_dir: str, epochs: int, dry_run: bool = False) -> bool:
    ...
    run_name = f"{config['model']}_{version}_{config['dataset']}_s{config['seed']}"
